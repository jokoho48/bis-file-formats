...
namespace BIS.WRP
{
    public class OPRW
    {
        ...
        {
            BinaryReaderEx input = new BinaryReaderEx(s);
            Read(input);
        ...
        {
            string fileSig = input.ReadAscii(4);
            ...
            Geography = new QuadTree<GeographyInfo>(LandRangeX, LandRangeY, input,
                (src, off) => BitConverter.ToInt16(src, off), 2);
            //if(version<19) transformOldWaterInformation

            int soundMapCoef = 1; //ToDo: this is read from config
            SoundMap = new QuadTree<byte>(LandRangeX * soundMapCoef, LandRangeX * soundMapCoef, input,
                (src, off) => src[off], 1); //both landRangeX are correct. no mistake

            Mountains = input.ReadArray(inp => new Vector3P(inp));

            Materials = new QuadTree<short>(LandRangeX, LandRangeY, input, (src, off) => BitConverter.ToInt16(src, off),
                2);
            ...
            byte[] major = new byte[nMaterials];
            ...
            ObjectOffsets = new QuadTree<int>(LandRangeX, LandRangeY, input,
                (src, off) => BitConverter.ToInt32(src, off), 4);
            int sizeOfObjects = input.ReadInt32();
            MapObjectOffsets = new QuadTree<int>(LandRangeX, LandRangeY, input,
                (src, off) => BitConverter.ToInt32(src, off), 4);
